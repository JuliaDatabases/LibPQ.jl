...
function test_data_typed_parsing()
    return [
        ("3", UInt, UInt(3)),
...
@testset "Online" begin
    ...
    @testset "Results" begin
        ...
        #                 @testset for (test_str, data) in test_data_parsing()
        #                     result = execute(
        #                         conn,
        #                         "SELECT $test_str;";
        #                         binary_format=binary_format,
        ...
        #                 for (test_str, typ, data) in test_data_typed_parsing()
        #                     result = execute(
        #                         conn,
        #                         "SELECT $test_str;",
        ...
        @testset "getindex(::Column) performance" begin
            # @testset "$value" for (value, result) in test_data_parsing()
            ...
            #     execute(
            #         conn, "INSERT INTO libpqjl_test (my_string_column) VALUES ($value)"
            #     )

            #     # Get the column.
            #     result = execute(
            #         conn, "SELECT my_string_column FROM libpqjl_test"
            #     )
            ...
            @testset "$value, bin=$bin_fmt" for
                ...
                result = execute(conn, "
                    CREATE TEMPORARY TABLE libpqjl_test (
                        id bigint,
                        my_column $pg_type_string
                    );
                ")

                # Put some data in the table.
                execute(
                    conn, "INSERT INTO libpqjl_test (my_column) VALUES ($value)"
                )

                # Get the column.
                result = execute(
                    conn, "SELECT my_column FROM libpqjl_test"; binary_format=bin_fmt,
                )
