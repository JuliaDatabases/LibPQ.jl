...
@testset "Online" begin
    ...
    @testset "Results" begin
            ...
            @testset "Parsing" begin
                ...
                @testset for binary_format in (LibPQ.TEXT, LibPQ.BINARY)
                    @testset "Default Types" begin
                        ...
                        @testset for (test_str, data) in test_data_1()
                            result = execute(
                                conn,
                                "SELECT $test_str;";
                                binary_format=binary_format,
                    ...
                    @testset "Specified Types" begin
                        ...
                        for (test_str, typ, data) in test_data_2()
                            result = execute(
                                conn,
                                "SELECT $test_str;",
        ...
        @testset "getindex(::Column) performance" begin
            @testset "$value" for (value, result) in test_data_1()
            ...
            @testset "$value" for (value, pg_type_string) in [
                (5.0, "float8"),
                (5f0, "float4"),
                (Int64(3), "int8"),
                (Int32(3), "int4"),
                ...
                # Get the column.
                result = execute(
                    conn, "SELECT my_column FROM libpqjl_test"; binary_format=true,
                )
